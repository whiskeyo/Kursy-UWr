...
(define (mlookup x xs)
  (cond
    ...
    [(eq? (mcar (mcar xs)) x) (match (mcar (mcdr (mcar xs)))
...
(define (opt-env program env)
  (define (help program)
    (match program
      [(op s l r)            (append (opt-env l)
                                     (opt-env r))]
      [(let-expr x e1 e2)    (append (opt-env e1)
                                     (garbage-collector x (opt-env e2)))]
      [(letrec-expr x e1 e2) (append (garbage-collector x (opt-env e1))
                                     (garbage-collector x (opt-env e2)))]
      [(variable x)          (list x)]
      [(if-expr b t e)       (append (opt-env b)
                                     (opt-env t)
                                     (opt-env e))]
      [(cons-expr l r)       (append (opt-env l)
                                     (opt-env r))]
  ...
  (define (godlike-env xs env)
    (cond
      [(null? xs) null]
      [else (mcons (mcons (car xs) (mlookup (car xs) env))
                   (godlike-env (cdr xs) env))]))
